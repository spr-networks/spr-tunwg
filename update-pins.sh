...
# lookups (registry manifest inspect, go.dev, git ls-remote). Review with `git diff`.
...
GO_MINOR=1.25
TUNWG_REPO=https://github.com/ntnj/tunwg.git
...
# Latest tunwg release tag (vYY.MM.DD+shorthash), pinned by its full commit hash.
read -r TUNWG_COMMIT TUNWG_VERSION < <(
  git ls-remote --tags "$TUNWG_REPO" 'refs/tags/v*' \
    | sed -e 's|refs/tags/||' -e 's|\^{}$||' | sort -u -V -k2,2 | tail -1)
[ -n "$TUNWG_COMMIT" ] && [ -n "$TUNWG_VERSION" ] || { echo "failed to resolve tunwg tag" >&2; exit 1; }
...
# tunwg (github.com/ntnj/tunwg) pinned to release tag ${TUNWG_VERSION} by full commit hash.
TUNWG_VERSION=${TUNWG_VERSION}
TUNWG_COMMIT=${TUNWG_COMMIT}
EOF
