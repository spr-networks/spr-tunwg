...
ARG TUNWG_COMMIT=bbd247bd282ae81d06b7421b65eba3bf9ff04848
ARG TARGETARCH
...
# tunwg from source, pinned by full commit hash (see reproducible.env).
WORKDIR /src
RUN set -eux; \
    git init -q tunwg; \
    cd tunwg; \
    git remote add origin https://github.com/ntnj/tunwg.git; \
    git fetch -q --depth 1 origin "${TUNWG_COMMIT}"; \
    git checkout -q FETCH_HEAD; \
    [ "$(git rev-parse HEAD)" = "${TUNWG_COMMIT}" ]
RUN --mount=type=tmpfs,target=/root/go/ --mount=type=tmpfs,target=/root/.cache/ \
    cd /src/tunwg && CGO_ENABLED=0 go build -trimpath -ldflags "-s -w" -o /tunwg ./tunwg
WORKDIR /code
COPY code/ /code/
RUN --mount=type=tmpfs,target=/root/go/ --mount=type=tmpfs,target=/root/.cache/ \
    go build -trimpath -ldflags "-s -w" -o /tunwg_plugin /code/
...
ARG TUNWG_VERSION=v25.11.15+bbd247b
ENV TUNWG_VERSION=${TUNWG_VERSION}
COPY scripts /scripts/
COPY --from=builder /tunwg /usr/local/bin/tunwg
COPY --from=builder /tunwg_plugin /
